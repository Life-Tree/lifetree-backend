...
@Injectable()
export class ImgValidatorReporter implements IReporter {

    constructor(
        @Inject('GoogleVision')
        private visionService: IVision, 
    ...
    private validateImages(images: Image[]): boolean{
        for (const image of images) {
            if(!this.visionService.isTree(image.getBase64())){
                return false;
            }
        }
        return true;
