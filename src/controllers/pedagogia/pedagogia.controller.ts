...
export class PedagogiaController {
    ...
    @Get('/:idMP')
    getPedagogia(@Param('idMP') idMP: string): Promise<MaterialPedagogico>{
    ...
    @Post()
    async crearPedagogia(@Body() mPedagogico: PedagogiaDTO): Promise<boolean>{
        let result = await this.pedagogiaManager.nuevoMP(mPedagogico.titulo,mPedagogico.descripcion,mPedagogico.dataURL);
        return result;
    }

    @Put(':idMP')
    modificarPedagogia(@Body() mPedagogico: PedagogiaDTO, @Param('idMP') idMP: string): string{
