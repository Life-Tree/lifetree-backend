...
export class ArbolesController {
    ...
    async crearArbol(@Body() arbol: ArbolDTO): Promise<string>{   
        ...
        let result = await this.arbolesManager.nuevoArbol(arbol.descripcion,arbol.imageSet,arbol.ubicacion.latitud,arbol.ubicacion.longitud, arbol.ubicacion.barrio);
        return result; 
    }
    ...
    addIntervencion(@Body() arbol: ArbolDTO, @Param('idArbol') arbolId: string): string{
        let result = this.arbolesManager.updateArbol(arbolId, arbol.descripcion,arbol.imageSet,arbol.ubicacion.latitud,
            arbol.ubicacion.longitud, arbol.ubicacion.barrio,arbol.estado,arbol.intervenciones,"IMAGE_CHANGE");        
    ...
    modificarArbol(@Body() arbol: ArbolDTO, @Param('idArbol') arbolId: string): string{
        ...
        let result = this.arbolesManager.updateArbol(arbolId, arbol.descripcion,arbol.imageSet,arbol.ubicacion.latitud,
            arbol.ubicacion.longitud, arbol.ubicacion.barrio, arbol.estado,arbol.intervenciones, "NO_IMAGE_CHANGE");        
